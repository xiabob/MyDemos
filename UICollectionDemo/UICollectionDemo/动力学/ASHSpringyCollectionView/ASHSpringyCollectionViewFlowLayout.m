...
-(id)init {
    ...
    self.itemSize = CGSizeMake(size.width-40, 150);
...
-(BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    //避免点击状态栏回到顶部时发生奇怪动力学效果
    if (fabsf(delta) > self.collectionView.frame.size.height) {
        return NO;
    }
    ...
    [self.dynamicAnimator.behaviors enumerateObjectsUsingBlock:^(UIAttachmentBehavior *springBehaviour, NSUInteger idx, BOOL *stop) {
        ...
        //这个会导致重新布局,prepareLayout方法被调用
        [self.dynamicAnimator updateItemUsingCurrentState:item];
    }];
    
    //普通情况下，collectionView在滑动，cell的展示是不需要什么特效的，它的UICollectionViewLayoutAttributes早就计算好了，直接用就行了，不需要重置Layout，重新布局item的属性。这里cell是有特效的，只不过效果的完成是通过动力学自动帮我们完成的，你不需要自己动手设置。
    return NO;
}
