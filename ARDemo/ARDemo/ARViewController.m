...
@interface ARViewController (ARViewControllerPrivate)<ARPopViewDelegate>
- (void) loadNFTData;
- (void) startRunLoop;
- (void) stopRunLoop;
...
@implementation ARViewController {
    ...
    BOOL hasShowPopView;
    ARPopView *popView;
    AREggLoadingView *loadingView;
    ARShowEggView *showEggView;
    ARScanView *scanView;
}
...
- (void) start2
{
    ...
    if (ar2VideoGetCParam(gVid, &cparam) < 0) {
        NSString *path = [[[NSBundle mainBundle] resourcePath] stringByAppendingString:@"/Data2/camera_para.dat"];
        const char *cparam_name = [[NSFileManager defaultManager] fileSystemRepresentationWithPath:path];
    ...
    glView = [[[ARView alloc] initWithFrame:self.view.bounds pixelFormat:kEAGLColorFormatRGBA8 depthFormat:kEAGLDepth16 withStencil:NO preserveBackbuffer:NO] autorelease]; // Don't retain it, as it will be retained when added to self.view.
    ...
    //set scan view
    if (!scanView) {
        scanView = [[ARScanView alloc] initWithFrame:self.view.bounds];
...
//                    NSLog(@"Tracked page %d.\n", detectedPage);
        ...
        for (ARMarker *marker in markers) {
            if ([marker isKindOfClass:[ARMarkerNFT class]]) {
                ...
                int tmpPage = detectedPage;
                if (hasShowPopView) {
                    tmpPage = -1;
                }
                
                //detectedPage指示探测到的marker，也就是markers的小标，对于要展示的模型对象，detectedPage有效，说明模型被正确展示出来
                [(ARMarkerNFT *)marker updateWithNFTResultsDetectedPage:tmpPage trackingTrans:trackingTrans];
                if (detectedPage >= 0 && marker.valid) {
                    hasDetected = TRUE;
                }
            } else {
                [marker update];
        ...
        if (hasDetected && !firstDetected) {
            firstDetected = YES;
            
            if (!showEggView) {
                showEggView = [[ARShowEggView alloc] initWithFrame:self.view.bounds];
            }
            [self.view addSubview:showEggView];
            
            if (!popView) {
                CGRect rect = self.view.frame;
                CGFloat width = rect.size.width * 0.7;
                CGFloat height = rect.size.height * 0.7;
                CGFloat x = rect.size.width * 0.15;
                CGFloat y = rect.size.height * 0.15;
                rect = CGRectMake(x, y, width, height);
                popView = [[ARPopView alloc] initWithFrame:rect];
            ...
            if (!loadingView) {
                ...
                CGFloat y = rect.size.height * 0.25;
                rect = CGRectMake(x, y, width, height);
                loadingView = [[AREggLoadingView alloc] initWithFrame:rect];
            }
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                           ...
                           dispatch_get_main_queue(), ^{
                               ...
                               [showEggView showEggWithDuratin:3 andCompleteBlock:^{
                                   [loadingView showLoadingViewWithDuratin:2 andCompleteBlock:^{
                                       [popView show];
                                   }];
                               }];
            });
